...
notifications, the inter-pane `loom` control bus, region capture, and the command palette.

So this list is **not** about re-paving basics. The guiding question is: *what sharpens the one
thing Loom is uniquely for — driving a fleet of CLI agents from one window?*

Everything here is **ADR-0001-safe**: Loom never parses pane *output*. Agent-driven signals
come in through the `loom` control bus (ADR-0007), which is an inbound, agent-pushed channel —
...
> ✅ notes below. The `loom` CLI gained a `status` subcommand and `ControlRequest` gained a
...
`loom attention`, raising its amber border. You type the answer **once** and it goes only to the
...
**Removed 2026-06-25:** the human broadcast bar (and its flagged-reply button / saved groups / snippets / stagger) was deleted — the user never used it and multi-agent work is cross-project. The agent-facing fan-out (`loom broadcast` / the `loom mcp` `broadcast` tool) is kept.
...
**The flow:** an agent sets its own short status — `loom status "running tests"` — shown in its
...
`loom` CLI (`src-tauri/src/cli.rs`). Clears on respawn.
...
repositions the full panes). `loom status [pane] <text…> | --clear` added to the CLI:
`loom status "running tests"` labels the calling pane; `loom status Cleo --clear` clears another's.
...
`broadcastTargets(ws)` instead of just the focused pane ("all of you read this") *(the broadcast bar has since been removed 2026-06-25; `broadcastTargets(ws)` now resolves to all live panes for the `loom broadcast` path)*; **"Open file…"**
...
- **System tray + global hotkey** to summon/hide the window. 🔴 ✅ shipped — a tray icon
  ...
  "Quit" emits `loom://quit` so the frontend flushes state first, like the close path).
- **Multi-window / tear-off panes** — currently a pane lives in one workspace in one window. 🔴 ✅ shipped
  ...
  `loom send` (the placeholder keeps a handle-routed registry entry); only scrollback `loom read` is
...
**✅ Built as:** a `loom hooks` subcommand (a *local* helper — no socket round-trip) that prints the
...
`loom attention` (#1), `UserPromptSubmit` → `loom status working` + `Stop` → `loom status` (clears) (#3).
...
"broadcast to the reviewers group", "flag myself blocked" — instead of shelling out to `loom`.
Mechanically a thin re-skin: the MCP tool handlers call the same relay the `loom` CLI does, so the
`loom` CLI and the MCP server become two front-ends to one bus.

**✅ Built as:** a `loom mcp` binary — a hand-rolled stdio MCP server (newline-delimited JSON-RPC,
...
relay is factored into `src/control_sock.rs`, shared with `loom` via `#[path]` so the two bins are
literally two faces of one relay. `flag_attention`/`set_status` default to the agent's own pane
(`$LOOM_PANE`). `loom mcp` ships beside `loom` (its dir is already on each pane's `PATH`) and its
absolute path is exposed as `$LOOM_BIN`. Register with `claude mcp add --transport stdio
loom -- loom mcp`. See [agent-mcp.md](agent-mcp.md). The hook adapter (B) stays for the
