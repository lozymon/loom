...
## Where Termhaus keeps its files

| What | Location (Linux) |
|---|---|
| Workspaces & per-pane intent | `~/.config/com.termhaus.app/workspaces.json` |
| Logs | `~/.config/com.termhaus.app/logs/` |
| Control-bus socket | `$XDG_RUNTIME_DIR/termhaus.sock` (fallback `/tmp/termhaus-<user>.sock`) |

Termhaus persists *intent*, not scrollback: on launch it rebuilds each workspace tree and
**respawns** every pane's command in its cwd. Terminals are not restored to their previous
output — that's by design.

### Reset to a clean slate
Quit Termhaus, then move `workspaces.json` aside:

```
mv ~/.config/com.termhaus.app/workspaces.json{,.bak}
```
...
**Garbled text, blank panes, or GPU glitches.** Termhaus renders on the **canvas**
...
## `th` / inter-pane control

**`th: ...` errors, or `th` not found.** `th` only works *inside* a pane Termhaus
launched — that's where `TERMHAUS_SOCK`, `TERMHAUS_PANE`, and the `PATH` entry for `th`
get injected. Check they're set:

```
echo "$TERMHAUS_PANE @ $TERMHAUS_SOCK"
command -v th
```

If `TERMHAUS_SOCK` is empty, you're not in a Termhaus pane (or you started a sub-shell
that scrubbed the environment).

**`th` can't reach the app / "connection refused".** The app must be running. A stale
socket file left by a crash is detected and replaced on next launch; if `th` still can't
connect, quit Termhaus fully and relaunch. The socket is per-user — `th` from a different
user account won't reach your panes.

**Commands target the wrong pane.** Panes are addressed by **display name**, not index.
Run `th list` to see current names; names can repeat across workspaces, so `th focus` /
`th send` resolve within reach of the caller's pane.

## Agent integration (MCP / hooks)

**MCP tools don't show up in Claude Code.** Confirm `th-mcp` is registered and that the
...
(`th hooks --install`) and confirm the agent's hook events fire. See
