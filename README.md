...
**A Linux-first desktop agent-first developer environment, built on real terminals** — a control room for driving and observing a fleet of CLI agents. It runs many real PTYs at once in resizable split grids and a left workspace rail (a graphical tmux / Terminator as the substrate): spawn a wall of terminals, watch which ones need you, and let one agent drive the others over the `loom` control bus (fanning a prompt to every pane with `loom broadcast`).

The engine stays generic — a pane is any pseudo-terminal running *any* command (a shell, `claude`, a dev server, `tail -f`, `vim`) — but the product leads with agents. The PTY hot path is byte-opaque (Loom never parses what a pane prints in the engine); agent awareness is pushed (bus/hooks/MCP) + kernel-derived, with an opt-in heuristic fallback ([ADR-0008](docs/adr/0008-agents-first-class-via-self-report.md), [ADR-0011](docs/adr/0011-heuristic-output-observer.md)).

> Status: **M0–M11 complete**, plus the post-v1 agentic arc (agents first-class, the coordination bus, the fleet panel). See [PLAN.md](PLAN.md) for the milestone build log, [AGENT_FIRST_PLAN.md](AGENT_FIRST_PLAN.md) for the agent-first reorientation, and [docs/adr/](docs/adr/) for the architecture decisions.
